...
import AppBootGuard from "@/components/AppBootGuard";
import FullPageLoading from "@/components/FullPageLoading";
import { lazyRouteLoaders } from "@/lib/navigation/routeLoaders";
import RoutePerformanceTracker from "@/components/RoutePerformanceTracker";
import { safeLogger } from "@/lib/logging/safeLogger";
import PilotOnboardingGuard from "@/components/onboarding/PilotOnboardingGuard";
import Landing from "./pages/Landing";
import Auth from "./pages/Auth";
import Onboarding from "./pages/Onboarding";
import PilotOnboardingPage from "./pages/onboarding/PilotOnboardingPage";
...
const BookingSuccessPage = lazy(lazyRouteLoaders.loadBookingSuccessPage);
const FrontDeskPage = lazy(lazyRouteLoaders.loadFrontDeskPage);
const AuditLogPage = lazy(lazyRouteLoaders.loadAuditLogPage);
const DemandDetailPage = lazy(lazyRouteLoaders.loadDemandDetailPage);
const RoomOperationDetailPage = lazy(lazyRouteLoaders.loadRoomOperationDetailPage);
const FolioPage = lazy(lazyRouteLoaders.loadFolioPage);
const LeadDetailPage = lazy(lazyRouteLoaders.loadLeadDetailPage);
const SetupWizardPage = lazy(lazyRouteLoaders.loadSetupWizardPage);
const AdminPricingPlansPage = lazy(lazyRouteLoaders.loadAdminPricingPlansPage);
const AdminFeaturesPage = lazy(lazyRouteLoaders.loadAdminFeaturesPage);
const AdminFaqsPage = lazy(lazyRouteLoaders.loadAdminFaqsPage);
const AdminTestimonialsPage = lazy(lazyRouteLoaders.loadAdminTestimonialsPage);
const AdminHowItWorksPage = lazy(lazyRouteLoaders.loadAdminHowItWorksPage);
const AdminIntegrationsPage = lazy(lazyRouteLoaders.loadAdminIntegrationsPage);
// Safety: keep heavy routes lazy-loaded to protect bundle size.

const queryClient = new QueryClient();

const App = () => {
  if (import.meta.env.MODE !== 'production') {
    safeLogger.debug('safety.notice', { message: 'Safety checks active. Run pnpm safety:check before merge.' });
  }
                ...
                <SelectedPropertyProvider>
                  ...
                  <Suspense fallback={<FullPageLoading />}>
                    ...
                    <Routes>
                      ...
                      <Route element={<AppBootGuard><PilotOnboardingGuard><Outlet /></PilotOnboardingGuard></AppBootGuard>}>
                        <Route path="/onboarding" element={<ProtectedRoute><Onboarding /></ProtectedRoute>} />
                        <Route path="/setup" element={<ProtectedRoute><SetupWizardPage /></ProtectedRoute>} />
                        <Route path="/pilot-onboarding" element={<ProtectedRoute><PilotOnboardingPage /></ProtectedRoute>} />
                        ...
                        <Route path="/m/housekeeping/task/:id" element={<Navigate to="/m/housekeeping" replace />} />
