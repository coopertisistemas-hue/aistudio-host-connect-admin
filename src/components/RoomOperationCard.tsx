...
interface RoomOperationCardProps {
  ...
  selectable?: boolean;
  selected?: boolean;
  onToggleSelect?: (roomId: string, selected: boolean) => void;
}

export const RoomOperationCard = ({ room, selectable = false, selected = false, onToggleSelect }: RoomOperationCardProps) => {
  ...
  return (
    <Card
      className={`group cursor-pointer hover:shadow-xl transition-all duration-300 hover:-translate-y-1 active:scale-[0.98] border-2 bg-gradient-to-br ${gradientClass} overflow-hidden relative ${selected ? 'ring-2 ring-primary' : ''}`}
      onClick={() => navigate(`/operation/rooms/${room.id}`)}
    >
      {selectable && (
        <label
          className="absolute left-3 top-3 z-20 inline-flex items-center gap-2 rounded-md bg-background/80 px-2 py-1 text-xs"
          onClick={(event) => event.stopPropagation()}
        >
          <input
            type="checkbox"
            checked={selected}
            onChange={(event) => onToggleSelect?.(room.id, event.target.checked)}
          />
          Selecionar
        </label>
      )}
      ...
      <CardHeader className="p-5 pb-3 flex flex-row items-start justify-between space-y-0 relative z-10">
        <div className="flex items-center gap-3 flex-1">
          ...
          <div className="flex-1 min-w-0">
            <CardTitle className="text-xl font-bold tracking-tight">Quarto {room.room_number}</CardTitle>
            <p className="text-xs text-muted-foreground font-medium truncate">{room.room_types?.name || 'Sem categoria'}</p>
          </div>
        </div>
      ...
      <CardContent className="p-5 pt-2 relative z-10">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl group-hover:bg-primary/10 group-hover:text-primary transition-colors">
